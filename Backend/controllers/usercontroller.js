...
const register = async (req, res) => {
  ...
  const updateUserDetails = async (req, res) => {
    try {
        ...
        const { _id } = req.user; // Assumes req.user contains authenticated user info
        const id = _id;

        // Destructure and extract fields from req.body
        const { 
            user_username, 
            user_email, 
            user_password, 
        ...
        // Find the user by ID
        let user1 = await user.findOne({ _id: id }); // Corrected the find query
        if (!user1) {
        ...
        if (user_username) user1.user_username = user_username;
        if (user_email) user1.user_email = user_email;
        if (user_fullname) user1.user_fullname = user_fullname;
        if (user_designation) user1.user_designation = user_designation;
        if (user_role) user1.user_role = user_role;
        if (user_status) user1.user_status = user_status;
        if (user_profile) user1.user_profile = user_profile;
        if (user_phone_number) user1.user_phone_number = user_phone_number;
